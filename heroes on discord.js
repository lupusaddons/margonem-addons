(function() {
    ...
    // NOWA FUNKCJA - taka sama jak w pierwszym kodzie
...
// ZNAJDŹ TĘ FUNKCJĘ (około linii 400-500) I ZAMIEŃ JĄ W CAŁOŚCI:
    ...
    try {
        ...
        for (const [npcId, npcData] of npcs) {
            try {
                ...
                if (heroWt !== null && heroWt !== undefined && getMobType(heroWt) === 'heroes') {
                    const heroKey = `${npcId}_${heroName}_${heroLevel}`;

                    if (!lastDetectedHeroes.has(heroKey)) {
                        lastDetectedHeroes.add(heroKey);
...
">
    ...
    <button style="
        ...
        font-weight: bold;
        line-height: 1;
    " id="hero-window-close" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='#ddd'">×</button>
</div>
...
function init() {
    // DODAJ EVENT LISTENER zamiast interval
    if (typeof window.API !== 'undefined' && window.API.addCallbackToEvent) {
        window.API.addCallbackToEvent('newNpc', function(npc) {
            ...
            try {
                ...
                if (npc && npc.d) {
                    heroName = npc.d.nick || npc.d.name;
                    heroLevel = npc.d.lvl || npc.d.elasticLevel;
                    heroWt = npc.d.wt;
                }

                if (heroWt !== null && heroWt !== undefined && getMobType(heroWt) === 'heroes') {
                    const heroKey = `${npc.id}_${heroName}_${heroLevel}`;
                    ...
                    if (!lastDetectedHeroes.has(heroKey)) {
                        ...
                        const additionalData = {
                            ...
                            finderName: getCurrentPlayerName(),
                            npcData: npc.d,
                            heroCoords: getHeroCoordinates(npc)
                        };
                ...
                console.error('Błąd przy przetwarzaniu nowego NPC:', error);
            }
        });
    }
